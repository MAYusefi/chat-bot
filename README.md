# چت‌بات حرفه‌ای سازمانی (Frontend + n8n Backend)

این پروژه یک چت‌بات مدرن و حرفه‌ای است که برای نمایش در GitHub/پرتفولیو طراحی شده و به ورک‌فلو n8n متصل می‌شود.

## ویژگی‌ها

- طراحی مدرن و واکنش‌گرا (Responsive)
- ساختار چندفایلی و تمیز (`HTML/CSS/JS` جدا)
- پشتیبانی کامل از فارسی/دری
...
## نکات انتشار در GitHub

- اگر پروژه را Public می‌کنید، هیچ `secret` یا `token` را داخل کد hardcode نکنید.
- برای نمایش بهتر به کارفرما، می‌توانید از GitHub Pages برای `index.html` استفاده کنید.

## پیشنهاد برای توسعه بعدی

- اضافه‌کردن احراز هویت کاربر (Login)
- ذخیره تاریخچه در دیتابیس واقعی به‌جای localStorage
- استریم پاسخ مدل (Streaming)
- اضافه‌کردن پنل ادمین و تحلیل گفتگو
